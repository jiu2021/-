...
- async
  ...
  - 数组
    ...
    `find()`、`findIndex()` 指定回调函数，第二个参数绑定this
    
    `fill(val,start,end)`  对象是填充内存地址非深拷贝
    ...
    `includes(val,searchStart)`能判断NaN `indexOf()`不能
    
    `flat()`默认只会“拉平”一层，如果想要“拉平”多层的嵌套数组，可以将`flat()`方法的参数写成一个整数，表示想要拉平的层数，默认为1。
    
    `flatMap()`方法的参数是一个遍历函数，该函数可以接受三个参数，分别是当前数组成员、当前数组成员的位置（从零开始）、原数组。只能展开一层。
